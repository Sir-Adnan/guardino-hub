...
export function Modal({
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center overflow-x-hidden overflow-y-auto p-3 sm:p-4">
      <div className="absolute inset-0 bg-black/55 backdrop-blur-[2.5px]" onClick={onClose} />
      <div
        className={cn(
          "relative my-6 min-w-0 w-full max-w-xl rounded-2xl border border-[hsl(var(--border))] bg-[linear-gradient(165deg,hsl(var(--card))_0%,hsl(var(--card))_52%,hsl(var(--muted))_100%)] shadow-2xl shadow-slate-900/20 max-h-[88dvh] overflow-hidden",
          className
        )}
      >
